...
class BottleneckResidualBlock(nn.Module):
    def __init__(self, in_channels=64,out_channels=64):
        self.conv0 = nn.Conv2d(in_channels, out_channels, kernel_size=3, bias=False)
...
class GenerativeModel(nn.module):
    def __init__(self, num_blocks=5, block_channels=64):
        ...
        self.after_blocks = nn.Sequential(
            nn.Conv2d(block_channels, 4*block_channels, kernel_size=3),
            nn.BatchNorm2d(color_channels)
        )
        self.upscaler = UpscalerBlock()
        self.final_conv = nn.Conv2d(4*block_channels, 3, kernel_size=9)
